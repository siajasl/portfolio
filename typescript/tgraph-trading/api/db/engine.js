...
/**
 * Returns a db engine for interacting with dB.
 */
...
const getUrlKey = (connectionType) => {
    ...
    const isKyc = connectionType.includes('kyc');
    if (isQuery) {
        if (isKyc) {
            return 'MONGODB_KYC_URL_READONLY';
        } else {
            return 'MONGODB_EXC_URL_READONLY';
        }
    } else {
        if (isKyc) {
            return 'MONGODB_KYC_URL_READWRITE';
        } else {
            return 'MONGODB_EXC_URL_READWRITE';
        }
    }
}
...
const getModels = (connection) => {
    ...
    Object.keys(schema).forEach((i) => {
        ...
        result[modelName] = connection.model(modelName, schema[i]);
    });
